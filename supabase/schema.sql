...
-- 7b. Question vetoes (hider rejects a question instead of answering it)
create table question_vetoes (
    id uuid primary key default gen_random_uuid(),
    question_id uuid not null references questions(id) on delete cascade unique,
    game_id uuid not null references games(id) on delete cascade,
...
create index idx_question_vetoes_game_id on question_vetoes(game_id);
create index idx_question_vetoes_question_id on question_vetoes(question_id);
